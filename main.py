...
def accept_user():
    ssn = request.form['ssnverification']
    ...
    if user:
        ...
        query3 = text("INSERT INTO users (Curser_ID, balance) VALUES (:Curser_ID, '0')")
        conn.execute(query3, {'Curser_ID': user[0], 'First_Name': user[1], 'Last_Name': user[2], 'Email': user[3], 'Password': user[4], 'Phone_Number': user[5], 'SSN': user[6]})
...
def ViewAccount():
    query = text("SELECT First_Name, Last_Name, Email, Password, Phone_Number, SSN FROM Cuser WHERE Cuser_ID = :Cuser_ID")
    balance = text("select balance from users where Cuser_ID = :cuser_ID")
    accountInfo = conn.execute(query, {'Cuser_ID': BankID}).fetchone()
    balance2 = conn.execute(balance, {'cuser_ID': BankID}).fetchone()

    return render_template('ViewAccount.html', AccountData=accountInfo, ballance=balance2)
...
def transfer_money():
    ...
    try:
        recipient_query = text("SELECT * FROM users WHERE Cuser_ID = :recipient_account_id")
        recipient = conn.execute(recipient_query, {'recipient_account_id': recipient_account_id}).fetchone()

        if recipient:
            user_query = text("SELECT balance FROM users WHERE Cuser_ID = :user_account_id")
            ...
            if user_balance >= amount:
                ...
                update_user_query = text("UPDATE users SET balance = :new_balance WHERE Cuser_ID = :user_account_id")
                ...
                update_recipient_query = text("UPDATE users SET balance = :new_balance WHERE Cuser_ID = :recipient_account_id")
...
def add_money():
    ...
    try:
        user_query = text("SELECT * FROM users WHERE Cuser_ID = :user_id")
        ...
        if user:
            ...
            update_query = text("UPDATE users SET balance = :new_balance WHERE Cuser_ID = :user_id")
